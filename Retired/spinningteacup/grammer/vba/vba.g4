...
chrStmt : CHR WS? LPAREN WS? valueStmt WS? RPAREN;
...
typeOfStmt : TYPEOF WS valueStmt (WS IS WS vbatype)?;
...
withStmt :
	WITH WS (implicitCallStmt_InStmt | (NEW WS vbatype)) endOfStatement
	block?
	END_WITH
;
...
asTypeClause : AS WS? (NEW WS)? vbatype (WS? fieldLength)?;
...
vbatype : (baseType | complexType) (WS? LPAREN WS? RPAREN)?;
