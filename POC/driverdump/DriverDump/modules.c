...
Pmodule_info add_new_module(
    IN HANDLE hProcess,
    IN struct myLDR_DATA_TABLE_ENTRY* ldr_entry)
{
    DWORD name_size;
...
BOOL read_ldr_entry(
    ...
    OUT struct myLDR_DATA_TABLE_ENTRY* ldr_entry,
    OUT wchar_t* base_dll_name)
{
    ...
    NTSTATUS status = NtReadVirtualMemory(
        ...
        ldr_entry_address,
        ldr_entry,
        sizeof(struct myLDR_DATA_TABLE_ENTRY),
        NULL);
    if (!NT_SUCCESS(status))
    {
        syscall_failed("NtReadVirtualMemory", status);
    ...
    IN BOOL is_lsass)
{
    ...
    // find the address of myLDR_DATA_TABLE_ENTRY
    ...
    struct myLDR_DATA_TABLE_ENTRY ldr_entry;
