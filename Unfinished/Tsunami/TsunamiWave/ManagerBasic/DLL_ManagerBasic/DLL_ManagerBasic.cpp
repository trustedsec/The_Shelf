...
#include "DLL_ManagerBasic.h"
#include "LIB_ManagerBasic.h"

HINSTANCE g_ManagerBasic_hModule = NULL;
	...
	LPVOID lpReserved) // reserved
{
	...
	__try
	{
		__try
		{
			...
			g_ManagerBasic_hModule = hinstDLL;
...
DWORD DLL_ManagerBasic_ExportedFunction(void)
{
	DWORD dwErrorCode = ERROR_SUCCESS;
	DWORD dwThreadId = 0;
	...
	__try
	{
		__try
		{
			DBGPRINT(DEBUG_VERBOSE, TEXT("DLL_ManagerBasic_ExportedFunction() start"));
		...
		__finally
		{
			...
			DBGPRINT(DEBUG_VERBOSE, TEXT("DLL_ManagerBasic_ExportedFunction() end"));
		} // end finally
	} // end try-except
	__except (EXCEPTION_EXECUTE_HANDLER)
	{
