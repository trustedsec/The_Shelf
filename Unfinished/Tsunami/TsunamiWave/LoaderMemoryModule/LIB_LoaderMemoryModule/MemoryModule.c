...
*/
LPVOID MemoryDefaultAlloc(
	IN	LPVOID lpAddress,
	IN	SIZE_T dwSize,
	IN	DWORD dwAllocationType,
	IN	DWORD dwProtect,
 ...
 * This is the default as used by MemoryLoadLibrary.
 */
HCUSTOMMODULE MemoryDefaultLoadLibrary(
 ...
 * This is the default as used by MemoryLoadLibrary.
 */
FARPROC MemoryDefaultGetProcAddress(
	IN	HCUSTOMMODULE hModule,
 ...
 * This is the default as used by MemoryLoadLibrary.
 */
BOOL MemoryDefaultFreeLibrary(
	IN	HCUSTOMMODULE hLibModule,
	IN	LPVOID lpUserData
);
...
typedef struct {
    ...
    CustomFreeFunc free;
    CustomLoadLibraryFunc fmLoadLibrary;
    CustomGetProcAddressFunc fpGetProcAddress;
    CustomFreeLibraryFunc fpFreeLibrary;
...
static inline LPVOID
OffsetPointer(LPVOID data, ptrdiff_t offset) {
    return (LPVOID) ((uintptr_t) data + offset);
}
...
#ifdef _WIN64
static void
FreePointerList(POINTER_LIST *head, CustomFreeFunc fpFreeMemory, void *lpUserData)
{
    POINTER_LIST *node = head;
    while (node) {
        POINTER_LIST *next;
        fpFreeMemory(node->address, 0, MEM_RELEASE, lpUserData);
        next = node->next;
        free(node);
        node = next;
    }
...
FinalizeSection(PMEMORYMODULE module, PSECTIONFINALIZEDATA sectionData) {
    ...
    if (sectionData->characteristics & IMAGE_SCN_MEM_DISCARDABLE) {
        ...
        if (sectionData->address == sectionData->alignedAddress &&
            ...
            module->free(sectionData->address, sectionData->size, MEM_DECOMMIT, module->lpUserData);
        }
        return TRUE;
    }
...
BuildImportTable(PMEMORYMODULE module)
{
    ...
    for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
        ...
        FARPROC *funcRef;
        HCUSTOMMODULE *tmp;
        HCUSTOMMODULE handle = module->fmLoadLibrary((LPCSTR) (codeBase + importDesc->Name), module->lpUserData);
        if (handle == NULL) {
        ...
        if (tmp == NULL) {
            module->fpFreeLibrary(handle, module->lpUserData);
        ...
        for (; *thunkRef; thunkRef++, funcRef++) {
            if (IMAGE_SNAP_BY_ORDINAL(*thunkRef)) {
                *funcRef = module->fpGetProcAddress(handle, (LPCSTR)IMAGE_ORDINAL(*thunkRef), module->lpUserData);
            } else {
                PIMAGE_IMPORT_BY_NAME thunkData = (PIMAGE_IMPORT_BY_NAME) (codeBase + (*thunkRef));
                *funcRef = module->fpGetProcAddress(handle, (LPCSTR)&thunkData->Name, module->lpUserData);
            }
            if (*funcRef == 0) {
                result = FALSE;
                break;
...
{
    UNREFERENCED_PARAMETER(lpUserData);
    return (HCUSTOMMODULE)LoadLibraryA(lpLibFileName);
}
...
)
{
    ...
    if (!CheckSize(dwLibraryBufferSize, sizeof(IMAGE_DOS_HEADER))) {
        return NULL;
    }
    dos_header = (PIMAGE_DOS_HEADER)lpLibraryBuffer;
    if (dos_header->e_magic != IMAGE_DOS_SIGNATURE) {
        SetLastError(ERROR_BAD_EXE_FORMAT);
        return NULL;
    }

    if (!CheckSize(dwLibraryBufferSize, dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS))) {
        return NULL;
    }
    old_header = (PIMAGE_NT_HEADERS)&((const unsigned char *)(lpLibraryBuffer))[dos_header->e_lfanew];
    ...
    for (i=0; i<old_header->FileHeader.NumberOfSections; i++, section++) {
        size_t endOfSection;
        if (section->SizeOfRawData == 0) {
            // Section without lpLibraryBuffer in the DLL
    ...
    code = (unsigned char *)fpAllocMemory(
    ...
    if (code == NULL) {
        ...
        code = (unsigned char *)fpAllocMemory(NULL,
            alignedImageSize,
            MEM_RESERVE | MEM_COMMIT,
            PAGE_READWRITE,
            lpUserData);
    ...
    while ((((uintptr_t) code) >> 32) < (((uintptr_t) (code + alignedImageSize)) >> 32)) {
        ...
        if (!node) {
            fpFreeMemory(code, 0, MEM_RELEASE, lpUserData);
            FreePointerList(blockedMemory, fpFreeMemory, lpUserData);
            SetLastError(ERROR_OUTOFMEMORY);
            return NULL;
        }
    ...
    result->alloc = fpAllocMemory;
    result->free = fpFreeMemory;
    result->fmLoadLibrary = fmLoadLibrary;
    result->fpGetProcAddress = fpGetProcAddress;
    result->fpFreeLibrary = fpFreeLibrary;
    result->lpUserData = lpUserData;
    result->pageSize = sysInfo.dwPageSize;
#ifdef _WIN64
    result->blockedMemory = blockedMemory;
#endif

    if (!CheckSize(dwLibraryBufferSize, old_header->OptionalHeader.SizeOfHeaders)) {
        goto error;
    }

    // commit memory for headers
    headers = (unsigned char *)fpAllocMemory(code,
    ...
    if (!CopySections((const unsigned char *) lpLibraryBuffer, dwLibraryBufferSize, old_header, result)) {
        goto error;
    }

    // adjust base address of imported lpLibraryBuffer
...
)
{
    ...
    if (HIWORD(lpProcName) == 0) {
        ...
        if (LOWORD(lpProcName) < exports->Base) {
            SetLastError(ERROR_PROC_NOT_FOUND);
            return NULL;
        }

        idx = LOWORD(lpProcName) - exports->Base;
        ...
        found = (const struct ExportNameEntry*) bsearch(&lpProcName,
...
{
    PMEMORYMODULE module = (PMEMORYMODULE)hLibModule;
    ...
    if (module->modules != NULL) {
        ...
        for (i=0; i<module->numModules; i++) {
            if (module->modules[i] != NULL) {
                module->fpFreeLibrary(module->modules[i], module->lpUserData);
            }
        }
    ...
    if (module->codeBase != NULL) {
        ...
        module->free(module->codeBase, 0, MEM_RELEASE, module->lpUserData);
    }

#ifdef _WIN64
    FreePointerList(module->blockedMemory, module->free, module->lpUserData);
#endif
